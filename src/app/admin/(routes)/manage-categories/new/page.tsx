...
const NewCategoryForm = () => {
  ...
  const checkForDuplicate = async (name: string, slug: string) => {
    try {
      const { categories } = await fetchCategories({ name, slug });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const isDuplicate = await checkForDuplicate(formData.name, formData.slug);
      if (isDuplicate) {
        setError("Category name or slug already exists.");
        return;
      }
      ...
      await addCategory(JSON.parse(JSON.stringify({ ...formData, image_url: compressedImageUrl })));
      ...
      if (errorMessage.includes("null prototypes") || errorMessage.includes("Only plain objects")) {
        ...
        setError("Failed to create category." + err);
      }
  ...
  return (
    <div className="max-w-md mx-auto p-4 shadow-md rounded-md">
      ...
      <h1 className="text-4xl font-semibold flex items-center my-10">
        <MdInsertEmoticon className="h-6 w-6 mr-2" /> Add New Category
      </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="slug" className="flex items-center my-4">
            <ArchiveIcon className="h-5 w-5 mr-2" /> Slug
          </label>
          <Input
            id="slug"
            name="slug"
