...
import { updateProduct, fetchProductById } from "@/actions/products";
...
const UpdateProductForm = ({ params }: { params: Promise<{ id: string }> }) => {

    const router = useRouter();
    const [formData, setFormData] = useState<Omit<FrozenProduct, 'createdAt'>>({
        id: 0,
        name: "",
        description: "",
        price: 0,
    ...
    useEffect(() => {
        const getProduct = async () => {
            ...
            const product: (FrozenProduct | null) = await fetchProductById(id)
            ...
            setFormData({
                ...
                detailed_images: product?.detailed_images || [],
            ...
            // Set detailed images state if any
            if (product?.detailed_images) {
                const formattedImages: { file?: File; url?: string; preview?: string; mode: 'url' | 'upload' }[] = product.detailed_images.map(url => ({
                    file: undefined,
                    url,
                    preview: url,
                    mode: 'url'
                }));
                setDetailedImages(formattedImages);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const updatedProduct = await updateProduct(JSON.parse(JSON.stringify(productData)));
    ...
    return (
        <div className="max-w-lg mx-auto p-6 shadow-md rounded-md">
            <h2 className="text-2xl font-semibold mb-4">Update Product: {formData?.id}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" disabled={isSubmitting || !formData.name || !formData.description || !formData.category || !formData.price || !formData.slug || (!mainImageURL && !mainImageFile)} className={`w-full ${isSubmitting ? 'opacity-50' : ''}`}>
                    {isSubmitting ? <LoaderIcon className="animate-spin" /> : <UpdateIcon />} Update Product
                </Button>
            </form>
        </div>
    );
